...
- watch/poll/wake ⇒ one async call + `wait_agent` that returns when the thing arrives. (wave0 root: registered a watch, polled, learned nothing, read git instead. that pattern is what this crate deletes.)
...
- library vs driver ! the lib exports the store, one future per agent (`Agent::run`), the mailbox, the request scheduler, the provider trait; it owns NO supervisor, NO process model, NO executor abstraction: it takes a tokio handle and returns futures; the tree is rows (paths, parent edges), fork is a store op, subtree shutdown is dropping futures under a token. the bin has a small driver (spawn agent futures, restart from inbox, serve web) that a consumer replaces with its own host. reason: exomonad already owns supervision (tidepool runtime session supervisor); an `ActorSystem` trait would abstract what is already data. the request scheduler is NOT actor scheduling (api capacity, 429 backoff, warmth priority) and stays in the lib.
...
- inference spend ! any test that calls the model is `#[ignore]`, run by hand, once, its trace recorded in `docs/findings.md`; no CI, no watch, no loop ever triggers inference. a red OFFLINE test on master between slices is acceptable as the next slice's contract.
