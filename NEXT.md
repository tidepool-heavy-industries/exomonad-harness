...
| core-compaction-v7 follow-up | preflighted Sol lead | base `763764d389763df6ab3ac0cc65b990c4b82fe1b7` | owns `crates/harness/src/compaction.rs`; children compaction-server-v7 (implementation) and compaction-contract-v7 (read-only) | contract child settled read-only; `ac02974` 1/1 not reviewed: typed_turn stub, wrong pin, missing returned-settings/user-message/successor invariants | same child repairs all PRD invariants, then one exact review; root owns Engine::run wiring |
