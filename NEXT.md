...
| strict finalize | adapter-finalize actor38; root Engine seam; reviewer actor5 | exact `6d78cc5` accepted as component-only, merged at `9e2f8a2`. Root Engine seam `be50cdd` Repair (configured collision), repaired at `dd3907b`, same reviewer Accepted exact `dd3907b` | Engine typed case 1/1, finalize filter 7/7, demo check; full vertical release remains open |
| offline replay + fake resident | repair leaf actor3; root durable seam; component reviewer actor9 | `ef954da` durable contract Accepted exact by actor6. Actor3's `18d3ee8` Store-backed ReplayProvider, fake resident fixture and reopen Engine/JobScheduler test passed replay 11/11 and demo check; exact seeded actor9 review reported no blocking defect, 2/2 provider tests. Root merged exact `18d3ee8` at `a70f12f` | integrated replay 11/11, full harness lib 113 passed/0 failed/2 ignored, demo binary 36/36, format/diff checks. Older `692adf4` was Repair (FIFO-only), not used as approval. Final long-cell/3-envelope/strict-final/restart release gate remains open. Old root demo branch `7c6dcca` and wakeup repair `5f9fea6` remain unmerged |
...
| release integration | root + vertical-gate leaf actor10; read-only audit actor11; reviewers actor12/13 | `a6ac194` scaffolded compiling `crates/harness/tests/adapter_readiness.rs`; live-source admission auto-checkpointed root NEXT.md at actual branch base `50eeb60` and root corrected both children. Leaf owns only the test file; audit owns no files | audit reported no structural blocker but misstated envelope timing; root corrected leaf: a request captured at wait_requested(n) cannot include an envelope inserted afterward; insert env1/2/3 while responses 2/3/4 gated, then assert admission in requests 3/4/5. Root withheld first candidate `b7632a3` despite an incorrect Accepted review: final-with-pending can loop into an unprovided sixth request. Leaf repaired at `53a219b`, focused 1/1 and demo check; reviewer13 Accepted exact tip on corrected actual base `50eeb606fc346c5cab105174a97617577713b5b3` (request20). Root merged exact leaf with `--no-commit` and integrated checks pass: harness lib 113 passed/2 ignored, adapter gate 1/1, demo 36/36, demo check, fmt and diff checks. Integration commit below. This closes the offline harness-side vertical gate, **not** adapter-host wiring or held live inference. |
...
- Adapter continuation (2026-09-25): root integration `a70f12ffe543ada826a42863bce7b977712c476e` merges exact actor3 candidate `18d3ee85c7251472bca0408933f8f6f528fe2e3b` over accepted `ef954da` durable contract; integrated replay 11/11, harness lib 113 passed/0 failed/2 ignored, demo binary 36/36. Root scaffolded the final gate as compiling `crates/harness/tests/adapter_readiness.rs` at `a6ac1940dc7e7f42833ec5d6d53828ccb38c4d0f` (`cargo check -p harness --tests` passed); live-source admission auto-checkpointed NEXT.md at actual child base `50eeb606fc346c5cab105174a97617577713b5b3` and root corrected leaf actor10/read-only audit actor11. Leaf owns only the test file; first expected reply is admission checkpoint, then 1/1 candidate; audit's is barrier report. `exomonad/adapter-readiness/wave-1/branches/adapter-replay` at `692adf43b513143116e265f4b45729c784768b71` retains the older 8/8 FIFO fixture but was Repair (not Store-backed), **not merged separately**. `adapter-replay-consumer` at `7c6dcca5baf3748a8f77494271d2585979236f07` contains root demo driver consumer plus older replay fixture; same-reviewer recheck was pending at redeploy, **not accepted or merged**. `exomonad/adapter-replay-combined-review/branches/repair-replay-wait` at `5f9fea6876d6a40b209db3da049219b28aa49f09` adds a replay request-wakeup repair atop `7c6dcca`, **not reviewed or merged**. Preserve these branches; no live inference authorized. Long-cell/3-envelope/strict-final/restart vertical gate still open.
- Vertical gate leaf `exomonad/adapter-readiness/vertical-gate-20260925/branches/vertical-gate-leaf` at `53a219b11f5ef80c8375145a140c67e48e4161f2` contains only `crates/harness/tests/adapter_readiness.rs` over actual base `50eeb606fc346c5cab105174a97617577713b5b3`; reviewer13 Accepted exact candidate on corrected base in request20. Root merged it into the pending `integrate(adapter-vertical-gate)` commit and ran the integrated checks named above. Reviewer12 request18 may still settle but is not an integration gate. Root audit branch made no edits/checks.
