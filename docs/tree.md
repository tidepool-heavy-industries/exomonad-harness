...
- **wave2 (standalone, operator-drivable; still no tidepool)**: the harness stays pluggable and the demo provider becomes something the operator can actually run a small coding task in, alone, from the web page. (a) auth: device-code login + refresh behind `Auth`; the run stops reading codex's `auth.json`. the retired `~/dev/tidepool/exomonad/harness/src/provider/oauth.rs` is a working reference (read-only, same rule as codex: read for pitfalls, close, write own). (b) demo coding tools under a MINIMAL sandbox: `run` confined to the subtree's worktree cwd, rlimits (address space, cpu), wall timeout, output cap (exists), network off when the platform allows (`unshare`); `read{path}`; `edit` stays owned-confined. no tidepool code has any of this today (checked 2026-09-24: the only `setrlimit` in that tree is vendored codex), so it is new but small; use a crate if one fits (nudge `reinvents_library`). (c) `--tree` is the default; the operator drives root from the page; `ask` works end to end. acceptance = the operator runs one real multi-file task in a scratch repo without codex installed and without touching tidepool. interrupt_agent stays TODO.
