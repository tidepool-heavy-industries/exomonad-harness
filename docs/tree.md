...
- **correction wave (IN PROGRESS: (a)(b) integrated, item-2 live trace open, (c)(d) not started (provenance is decided, see `Store::append_items`), probe rescoped to a counter measurement; status table in `docs/correction-plan.md`; small tree, same node pattern; server/auth/web FROZEN)**: root ⇓ `core` ⇓ ≤2 leaves at a time. four items, in this order because all touch the loop: (a) collapse the engine to ONE run entry (durable head + new items + mailbox); delete the rest; no compat shims. (b) tools that run as jobs carry `async: true`; prove item 2 LIVE (slow `sleep`, model continues, wait_agent resumed by the result) and record the request bodies. (c) settings items: `configuration_update` as a positional item in the store, `set_effort` verb, fork strip list, effort pinned via the item not the request field; prove item 13 LIVE (child history = parent prefix + one update) and record cache counters. (d) `Compactor` per PRD shape (`CompactContext`, `Summary`, `NewWindow{carried}`) w/ `Server` only; run the unanswered-call experiment → findings.md. plus one findings-only probe, any leaf, no code: two consecutive requests through OUR builder, same `prompt_cache_key`, stable >1,024-token prefix; record both `usage` blocks redacted. the question is "does `cached_tokens` read >0 on the second request", yes or no. codex parity is NOT the criterion (Q4): compare headers/body order against codex's request builder in source only if the answer is no, and then as a diff of fields, never bytes. no wire capture exists or is required. also: provider trait toward PRD shape (typed `CallContext` w/ cancel token, bounded progress, `JobVerbs`) is allowed in this wave if (a)–(c) need it; hooks are NOT (wave1). interview answers are a deliverable this time: `docs/interviews.md`, one section per node. one instance to dogfood the design against, from the 2026-09-24 run: a parent sent a child a message, got a delivery receipt, and could not tell for 27 min whether the child ever saw it. in this harness that question has a store answer: a `send_message` envelope is an item; "presented" = the request it was appended to (or none yet); "answered" = the child's reply naming it. `list_agents` shows those three per pending message, and there is no receipt that means less than that.
...
## root (level 0)
- root scaffolds, spawns, integrates, reviews. root implements only the scaffold and the seams it retains; every bounded leaf is a child, and each Sol below it forks Lunas the same way rather than writing the leaf itself (wave3 root wrote leaf code, reviewed it, and integrated it, and spent 43% of its active time polling; a root that codes leaves is a root that is not watching the tree.)
